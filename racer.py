...
path_name = "C:/Users/Ali/Desktop/PROGRAMMING/PYTHON/GAMES/Racer/"

# Play background music
mixer.init()
mixer.music.load(f"{path_name}background.mp3")
mixer.music.play()
...
# Register Shapes
wn.register_shape(f"{path_name}road.gif")
wn.register_shape(f"{path_name}car.gif")
wn.register_shape(f"{path_name}roadrect.gif")
wn.register_shape(f"{path_name}heart.gif")
...
# Global Arrays
coin_frames = [f"{path_name}coin1.gif",f"{path_name}coin2.gif", f"{path_name}coin3.gif", f"{path_name}coin4.gif", f"{path_name}coin5.gif",f"{path_name}coin6.gif",f"{path_name}coin7.gif",f"{path_name}coin6.gif",f"{path_name}coin5.gif",f"{path_name}coin4.gif",f"{path_name}coin3.gif",f"{path_name}coin2.gif",]
colors = ["red", "black", "yellow", "green", "blue", "lightblue", "purple", "pink", "magenta", "cyan"]
enemycars = [f"{path_name}enemycar1.gif", f"{path_name}enemycar2.gif", f"{path_name}enemycar3.gif"]
...
class Coin(Turtle):
    def __init__(self, x, y):
        ...
        self.speed = common_speed
        self.penup()
        self.shape(f"{path_name}coin1.gif")
        self.animate()
        self.goto(x, y)
        self.width = 0
        self.height = 0
    ...
    def move(self):
        ...
        if player.isCollision2(coin):
            self.setx(1000)
            playsound(f"{path_name}coin.wav")
...
class Heart(Turtle):
    def __init__(self, x, y):
        ...
        self.speed = common_speed
        self.penup()
        self.shape(f"{path_name}heart.gif")
        self.goto(x, y)
        self.width = 0
        self.height = 0
    ...
    def move(self):
        ...
        if player.isCollision2(heart):
            self.setx(1000)
            playsound(f"{path_name}powerup.wav")
            lives.lives += 1
...
class Game():
    ...
    def restart(self):
        ...
        # Sound
        mixer.music.load(f"{path_name}background.wav")
        mixer.music.play()
    ...
    def lose(self):
        ...
        # Sound
        mixer.music.pause()
        playsound(f"{path_name}lose.wav")
        ...
        coins_collected_image = Sprite(f"{path_name}coin2.gif", -20, -45, 0, 0)
...
# Create Sprites
road = Sprite(f"{path_name}road.gif", 15, -30, 0, 0)

# Road Rectangles
rects = []
for i in range(0, 12):
     rects.append(Sprite(f"{path_name}roadrect.gif", -9, i * WIN_HEIGHT / 8, 0, 0))
     rects[i].color("white")
     rects[i].shapesize(1.7, 1)

player = Player(f"{path_name}car.gif", 0, -230)
...
# Coins Collected
Sprite(f"{path_name}coin2.gif", 130, 275, 0, 0)
coin_score = CoinScore(160, 270)

# Lives
Sprite(f"{path_name}heart.gif", 0, 287, 0, 0)
lives = Lives(40, 270)
...
while True:
    ...
    for enemy in enemies:
        ...
        if player.isCollision2(enemy):
            lives.lives -= 1
            enemy.setx(10000)
            playsound(f"{path_name}hurt.wav")
